...
<body>
    ...
    <div class="header">Print Report</div>
    <div class="header">Garment Types: {{ $print->garment_type ?? 'N/A' }}</div>
    <div style="text-align:right; margin-top:8px;">Date: {{ \Carbon\Carbon::parse($print->date)->format('d-m-Y') }}</div>
    ...
    <table style="margin-top:16px;">
        ...
        <tbody>
            ...
            @foreach($print->print_data as $row)
                <tr>
                    <td>{{ $serial++ }}</td>
                    <td>{{ $print->order->style_no ?? 'N/A' }}</td>
                    <td>{{ $row['color'] }}</td>
                    <td>{{ $row['order_qty'] }}</td>
