<x-layouts.app>
    <x-slot name="title">Edit Line Data</x-slot>
    <x-slot name="pageTitle">Edit Line Data</x-slot>

    <div class="card">
        <div class="card-body">
            <form id="form" action="{{ route('lines.update', $line->id) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="name" id="name"
                        value="{{ $line->name }}" placeholder="Enter your line name" required>
                    <p class="invalid-feedback"></p>
                </div>
                <div class="mb-3">
                    <label class="form-label">Status</label>
                    <select name="lineStatus" id="lineStatus" class="form-select" required>
                        <option value="" disabled selected>Select status...</option>
                        <option value="1" {{ $line->status === 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $line->status === 0 ? 'selected' : '' }}>Disable</option>
                        <p class="invalid-feedback"></p>
                    </select>
                    <p class="invalid-feedback"></p>
                </div>
                ...
                <a href="{{ route('lines.index') }}" class="btn btn-secondary">Cancel <i
                    ...
                    axios.post(form.attr("action"), formData, {
                        ...
                        .then(function(response) {
                            ...
                            if (response.data.status) {
                                window.location.href = "{{ route('lines.index') }}";
                            } else {
