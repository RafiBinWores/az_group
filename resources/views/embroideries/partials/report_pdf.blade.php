...
<body>
    ...
    <div class="header">Embroidery Report</div>
    <div class="header">Garment Types: {{ $embroidery->garment_type ?? 'N/A' }}</div>
    <div style="text-align:right; margin-top:8px;">Date: {{ \Carbon\Carbon::parse($embroidery->date)->format('d-m-Y') }}</div>
    ...
    <table style="margin-top:16px;">
        ...
        <tbody>
            @php
                $orderQty = 0; $cuttingQty = 0; $sendQty = 0; $receiveQty = 0;  $serial = 1;
            @endphp
            @foreach($embroidery->embroidery_data as $row)
                <tr>
                    <td>{{ $serial++ }}</td>
                    <td>{{ $embroidery->order->style_no ?? 'N/A' }}</td>
                    <td>{{ $row['color'] }}</td>
                    <td>{{ $row['order_qty'] }}</td>
