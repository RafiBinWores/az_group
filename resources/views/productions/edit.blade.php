<x-layouts.app>
    {{-- Page title --}}
    <x-slot name="title">Edit Print Report</x-slot>
    <x-slot name="pageTitle">Edit Print Report</x-slot>

    <div class="card">
        <div class="card-body">
            <form id="form" action="{{ route('prints.update', $print->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select name="order_id" id="style-select" class="form-select mt-1" autocomplete="off">
                        ...
                        @foreach ($orders as $order)
                            <option value="{{ $order->id }}" 
                                ...
                                @if($order->id == old('order_id', $print->order_id)) selected @endif>
                                {{ $order->style_no }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date" id="date" class="form-control mt-1" value="{{ old('date', $print->date ? date('Y-m-d', strtotime($print->date)) : '') }}">
                    <div class="error text-danger small mt-1"></div>
                </div>
    ...
    <script>
        ...
        const printData = @json($print->print_data);
        const selectedOrderId = '{{ old('order_id', $print->order_id) }}';
        const selectedGarmentType = '{{ old('garment_type', $print->garment_type) }}';

        // --- Helper: Find saved embroidery data for color ---
        function getSavedPrintData(color) {
            return (printData || []).find(e => e.color === color) || {};
        }
        ...
        function populateFields() {
            ...
            if (colors) {
                ...
                colors.forEach((row, idx) => {
                    ...
                    let saved = getSavedPrintData(row.color);
                    ...
                    div.innerHTML = `
                        ...
                        <div class="col-6 col-md-4 col-lg-2">
                            ...
                            <input type="number" readonly min="0" value="${row.qty}" class="form-control bg-soft-secondary" name="print_data[${idx}][order_qty]">
                        </div>
                        <div class="col-6 col-md-4 col-lg-2">
                            <span class="fw-semibold">Cutting Qty</span>
                            <input type="number" readonly min="0" value="${lastCuttingQty || 'N/A'}" class="form-control bg-soft-secondary" name="print_data[${idx}][cutting_qty]">
                        </div>
                        <div class="col-6 col-md-4 col-lg-2">
                            <span class="fw-semibold">Factory</span>
                            <input type="text" placeholder="Factory" class="form-control" name="print_data[${idx}][factory]" value="${saved.factory ?? ''}">
                        </div>
                        <div class="col-6 col-md-4 col-lg-2">
                            <span class="fw-semibold">Send</span>
                            <input type="number" min="0" placeholder="Send" class="form-control" name="print_data[${idx}][send]" value="${saved.send ?? ''}">
                        </div>
                        <div class="col-6 col-md-4 col-lg-2">
                            <span class="fw-semibold">Received</span>
                            <input type="number" min="0" placeholder="Received" class="form-control" name="print_data[${idx}][received]" value="${saved.received ?? ''}">
                        </div>
                    `;
