<x-layouts.app>
    {{-- Page title --}}
    <x-slot name="title">Create Wash Report</x-slot>
    {{-- Page header --}}
    <x-slot name="pageTitle">Create Wash Report</x-slot>


    {{-- Page Content --}}
    <div class="card">
        <div class="card-body">
            <form id="form" action="{{ route('washes.store') }}" method="POST" class="needs-validation" novalidate>
                @csrf
    ...
    @push('scripts')
        <script>
            const latestProdTotals = @json($latestProdTotals ?? []);

            document.getElementById('style-select').addEventListener('change', function() {
                // Garment Types (unchanged)
                ...
                if (colors) {
                    ...
                    colors.forEach((row, idx) => {
                        ...
                        const lastTotalOutput =
                            latestProdTotals[selectedOrderId] && latestProdTotals[selectedOrderId][
                            colorName] !== undefined ?
                            latestProdTotals[selectedOrderId][colorName] :
                            '';
                    ...
                    <div class="col-6 col-md-4 col-lg-2">
                        ...
                        <input type="number" readonly min="0" value="${row.qty}" class="form-control bg-soft-secondary" name="wash_data[${idx}][order_qty]">
                    </div>
                    <div class="col-6 col-md-4 col-lg-2">
                        <span class="fw-semibold">Output Qty</span>
                        <input type="text" readonly value="${lastTotalOutput !== '' ? lastTotalOutput : 'N/A'}" class="form-control bg-soft-secondary" name="wash_data[${idx}][output_qty]">
                    </div>
                    <div class="col-6 col-md-4 col-lg-2">
                        <span class="fw-semibold">Factory</span>
                        <input type="text" placeholder="Factory" class="form-control" name="wash_data[${idx}][factory]">
                    </div>
                    <div class="col-6 col-md-4 col-lg-2">
                        <span class="fw-semibold">Send</span>
                        <input type="number" min="0" placeholder="Send" class="form-control" name="wash_data[${idx}][send]">
                    </div>
                    <div class="col-6 col-md-4 col-lg-2">
                        <span class="fw-semibold">Received</span>
                        <input type="number" min="0" placeholder="Received" class="form-control" name="wash_data[${idx}][received]">
                    </div>
                `;
