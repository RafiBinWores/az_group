...
class ThemeCustomizer {
    ...
    saveConfig() {
        if (typeof(Storage) !== "undefined") {
            localStorage.setItem('adminto-theme-config', JSON.stringify(this.config));
        }
    }
    ...
    loadConfig() {
        if (typeof(Storage) !== "undefined") {
            const savedConfig = localStorage.getItem('adminto-theme-config');
            if (savedConfig) {
    ...
    initConfig() {
        ...
        // Apply the configuration to the body
        this.applyConfig();
        this.setSwitchFromConfig();
    }

    // Apply configuration to body attributes
    applyConfig() {
        ...
        if (this.config.sidebar.user) {
            this.body.setAttribute('data-sidebar-user', this.config.sidebar.user);
        } else {
    ...
    resetTheme() {
        ...
        // Clear saved configuration and save default
        if (typeof(Storage) !== "undefined") {
            localStorage.removeItem('adminto-theme-config');
        }
        this.saveConfig();
    }
