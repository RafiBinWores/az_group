...
class WashExport implements FromCollection, WithHeadings, WithTitle, WithEvents
{
    protected $wash;

    public function __construct(Wash $wash)
    {
        $this->wash = $wash;
    }
    ...
    public function collection()
    {
        ...
        foreach ($this->wash->wash_data as $row) {
            $orderQty = (int)($row['order_qty'] ?? 0);
            $outputQty = (int)($row['output_qty'] ?? 0);
            $send = (int)($row['send'] ?? 0);
            ...
            $rows[] = [
                'Serial No'         => $serial++,
                'Style No'          => $this->wash->order->style_no ?? 'N/A',
                'Garment Type'      => $this->wash->garment_type ?? 'N/A',
                'Color'             => $row['color'] ?? '',
                'Order Quantity'    => $orderQty,
                'Output Quantity'    => $outputQty,
                'Send'              => $send,
                'Receive'           => $receive,
                'Balance'           => $balance,
                'Remarks'           => $row['remarks'] ?? '',
        ...
        $rows[] = [
            ...
            'Output Quantity'  => $totalOutputQty,
            'Send'              => $totalSend,
    ...
    {
        $date = Carbon::parse($this->wash->date)->format('d-m-Y');
        return [
            ['A.Z Group'],
            ...
            [],
            ['', '', '', '', '', '', '', '', 'Date: ' . $date],
            ['Serial No', 'Style No', 'Garment Type', 'Color', 'Order Quantity', 'Production Quantity', 'Send', 'Receive', 'Balance', 'Remarks'],
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Borders for table
                $rowCount = count($this->wash->wash_data);
